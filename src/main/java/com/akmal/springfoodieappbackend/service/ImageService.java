...
/**
 * ImageService class is a {@link org.springframework.stereotype.Service} that abstracts the
 * business logic of managing {@link com.akmal.springfoodieappbackend.model.Image} entities from the
 * controller.
 *
...
public class ImageService {
  ...
  /**
   * Method is responsible for uploading new images with a given title by means of the underlying
   * file service provider. Thereafter, it prepares the {@link Image} entity object that contains
   * fields like url, key as well as title. Url and key are gotten from the {@link FileMetaData}
   * returned by the {@link FileService#upload(MultipartFile, FileService.FileType)}.
   *
  ...
  public void deleteImageByKey(String key) {
    final var image =
        this.imageRepository
            .findById(key)
            .orElseThrow(
                () -> new NotFoundException(String.format("Image with ID %s was not found", key)));
  ...
  /**
   * The method is responsible for deleting all the references to the particular image in the tables
   * like Restaurant, Categories and MenuItems to avoid violating FK constraints.
   *
  ...
  private void removeReferences(String imageId) {
    ...
    this.menuItemService.removeImageReferences(imageId);
  }
