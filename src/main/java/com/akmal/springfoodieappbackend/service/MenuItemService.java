...
public class MenuItemService {
  ...
  /**
   * The method is responsible for deleting all the image references for a given image ID.
   * It iterates through all the matching items and sets its thumbnail and full image fields to null
   * if they match the given id.
   *
   * @param imageId of an {@link com.akmal.springfoodieappbackend.model.Image} entity that is going to be removed.
   */
  @Transactional
  ...
  /**
   * The method is responsible for saving the menu item as a new object, to ensure that it is persisted as
   * a new object, we have to set the id to null regardless of what values the client has presented us.
   *
  ...
  public MenuItemDto save(MenuItemDto menuItemDto) {
    final var existingMenu = this.transactionRunner.runInTransaction(() -> this.getExistingMenu(menuItemDto.menuId()));

    final var mappedMenuItem = this.menuItemMapper.from(menuItemDto)
   ...
   * Firstly, it finds the existing menu and checks the ownership of the restaurant to which the menu belongs to.
   * Thereafter, maps the {@link MenuItemDto} to the POJO and persists it.
   *
   * @param menuItemDto {@link MenuItemDto} object.
  ...
  public MenuItemDto update(MenuItemDto menuItemDto, Long id) {
    ...
    final var existingMenuItem = this.menuItemRepository.findById(id)
            .orElseThrow(() -> new NotFoundException(String.format("MenuItem with ID %d was not found", id)));
  ...
  /**
   * The method is responsible for retrieving the existing menu item from the database
   * given the {@code menuId} argument and verifies the ownership of the current user
   * and the {@link com.akmal.springfoodieappbackend.model.Restaurant} entity the menu belongs to.
   *
   * @param menuId id of a {@link Menu} entity
   * @return {@link Menu} instance object.
   * @throws com.akmal.springfoodieappbackend.exception.InsufficientRightsException if the current user is not
   *                                                                                restaurant owner to which menu belongs to.
   */
  private Menu getExistingMenu(Long menuId) {
    final var existingMenu = this.menuRepository.findById(menuId)
            .orElseThrow(() -> new NotFoundException(String.format("Menu with ID %s was not found", menuId)));
