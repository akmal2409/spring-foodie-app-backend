...
/**
 * The Service class that manages the data transformation of the {@link
 * com.akmal.springfoodieappbackend.model.Menu} entity.
 *
...
public class MenuService {
  ...
  /**
   * The method is responsible for finding all {@link Menu} entity objects by {@code restaurantId}
   * and returns paginated result for a given page and size.
   *
  ...
  public Page<MenuDto> findAllByRestaurantId(long restaurantId, int page, int size) {
    final var restaurant =
        this.transactionRunner.runInTransaction(() -> this.findRestaurantById(restaurantId));
  ...
  public MenuDto findById(long id) {
    final var menu = this.menuRepository.findById(id).orElse(null);

    return this.menuMapper.toDto(menu);
  }

  /**
   * The method is responsible for saving a new {@link Menu} entity to the database. Requires the
   * restaurant to be present and the current user being the owner of the resource.
   *
   * @param menuDto {@link MenuDto} object
   * @return {@link MenuDto} saved object.
   * @throws com.akmal.springfoodieappbackend.exception.InsufficientRightsException if the user is
  ...
  public MenuDto save(MenuDto menuDto) {
    final var restaurant =
        this.transactionRunner.runInTransaction(
            () -> this.findRestaurantById(menuDto.restaurantId()));

    final var mappedMenu = this.menuMapper.from(menuDto).withRestaurant(restaurant);
  ...
  /**
   * Method is responsible for updating the existing {@link Menu} entity in the database.
   *
   * @param menuDto {@link MenuDto} object
  ...
  public MenuDto update(MenuDto menuDto, long id) {
    ...
    final var existingMenu =
        this.menuRepository
            .findById(id)
            .orElseThrow(
                () -> new NotFoundException(String.format("Menu with ID %d was not found", id)));

    this.restaurantService.verifyUserIsOwner(restaurant);
    final var mappedMenu =
        this.menuMapper.from(menuDto).withRestaurant(restaurant).withId(existingMenu.getId());
  ...
  private Restaurant findRestaurantById(long id) {
    return this.restaurantRepository
        .findById(id)
        .orElseThrow(
            () -> new NotFoundException(String.format("Restaurant with id %d was not found", id)));
  }
}
