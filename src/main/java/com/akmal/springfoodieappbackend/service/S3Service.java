...
public class S3Service implements FileService {
  ...
  public FileMetaData upload(MultipartFile file, FileType type) {
    FileValidatorFactory.getInstance(type)
            .validate(file);

    final var fileExtension = StringUtils.getFilenameExtension(file.getOriginalFilename());

    final var key = String.format("%s-%s.%s", UUID.randomUUID().toString(), type, fileExtension);
    ...
    try {
      ...
      log.error("Exception occurred uploading files to S3 bucket {}. Message: {}", this.bucketName, e.getMessage());
  ...
  public void deleteById(String key) {
    try {
      ...
      log.error("Exception occurred deleting file with key {} in S3 bucket {}. Message: {}", key, this.bucketName, e.getMessage());
